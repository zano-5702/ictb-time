...
class WorkTimeAdapter extends utils.Adapter {
    ...
    async onReady() {
        // Lade Kunden und Mitarbeiter aus der nativen Konfiguration
        ...
        // Aktualisiere Objekte für Kunden und Mitarbeiter
        ...
        await createTimeTrackingStates();

        // Abonniere Geofence-Änderungen von traccar (zum Beispiel für Device 1)
    ...
    async updateCustomerObjects() {
        // Abrufen der bereits existierenden Kundenobjekte unter ictb-time.0.kunden.*
    ...
    async onStateChange(id, state) {
        ...
        // Reagiere nur auf den Geofence-Status des Beispielgeräts (z. B. traccar.0.devices.1.geofences_string)
        const match = id.match(/(traccar\.0\.devices\.\d+)\.geofences_string/);
        if (!match) return;
        const deviceKey = match[1];

        // Prüfe, ob für diesen Device ein Mitarbeiter definiert ist
        const employee = this.config.employees[deviceKey];
        if (!employee) {
            this.log.warn(`Kein Mitarbeiter für ${deviceKey} definiert.`);
            return;
        }
        // Führe die Verarbeitung für Geofence-Änderungen durch
        processGeofenceChange({ state, oldState: {} }).catch(err => this.log.error(err));
    }
    ...
    async updateAggregates(employee, logEntry) {
        // Platzhalter: Hier Aggregationslogik einfügen
...
// --- Funktionen, die nicht Teil der Klasse sind ---

// Hilfsfunktion: createStateIfNotExists (nutzt createStateAsync, um vorhandene States nicht zu überschreiben)
async function createStateIfNotExists(id, initialValue, commonObj) {
    await adapter.createStateAsync(id, initialValue, false, commonObj);
}

// Funktion zum Anlegen der TimeTracking-States in 0_userdata.0.TimeTracking
async function createTimeTrackingStates() {
    // Array mit Kundendaten – hier statisch, kann aber auch dynamisch aus der Konfiguration geladen werden.
    const customers = [
        {
            name: "HW2-9-Wohlen",
    ...
    for (let customer of customers) {
        ...
        await createStateIfNotExists(`${basePath}.time_day`, 0, {
            name: "Time (Day)",
            type: "number",
            role: "value",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.time_week`, 0, {
            name: "Time (Week)",
            type: "number",
            role: "value",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.time_month`, 0, {
            name: "Time (Month)",
            type: "number",
            role: "value",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.time_year`, 0, {
            name: "Time (Year)",
            type: "number",
            role: "value",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.lastEnter`, "", {
        ...
        await createStateIfNotExists(`${basePath}.work_log`, "[]", {
            name: "Work Log (JSON)",
            type: "string",
            role: "json",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.work_report`, "", {
            name: "Work Report",
            type: "string",
            role: "text",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.hourly_rate`, hourlyRate, {
            name: "Hourly Rate",
            type: "number",
            role: "value",
            read: true,
        ...
        await createStateIfNotExists(`${basePath}.customerAddress`, address, {
        ...
        adapter.log.info(`TimeTracking-States für "${geofenceName}" angelegt oder aktualisiert.`);
    }

    // Optionale States für manuelle Büroarbeit
    await createStateIfNotExists(`0_userdata.0.TimeTracking.Manual.working`, false, {
    ...
    await createStateIfNotExists(`0_userdata.0.TimeTracking.Manual.selectedID`, "", {
    ...
    await createStateIfNotExists(`0_userdata.0.TimeTracking.Manual.lastEnter`, "", {
    ...
    await createStateIfNotExists(`0_userdata.0.TimeTracking.Manual.work_report`, "", {
...
async function processGeofenceChange(obj) {
    // A) Neuer Wert und alter Wert
    const newID = (obj.state.val || "").trim();
    const oldID = (obj.oldState && obj.oldState.val ? obj.oldState.val : "").trim();
    
    adapter.log.info(`Geofence Änderung erkannt: alt="${oldID}" - neu="${newID}"`);
    
    // B) Wartezeit (hier ca. 5 Minuten 10 Sekunden)
    await sleep(310000);
    
    // C) Aktuellen Wert erneut abfragen
    const currentState = await adapter.getStateAsync("traccar.0.devices.1.geofences_string");
    const stableID = (currentState && currentState.val ? currentState.val : "").trim();
    
    // D) Wenn sich der Wert innerhalb der Wartezeit geändert hat, Abbruch
    if (stableID !== newID) {
        adapter.log.info(`Geofence stabilisiert sich nicht: ursprünglicher Wert "${newID}" vs. aktueller Wert "${stableID}". Abbruch.`);
        return;
    }
    
    // E) Falls ein vorheriger Geofence vorhanden ist und sich geändert hat:
    if (lastGeofenceID && lastGeofenceID !== newID) {
        adapter.log.info(`Verlasse Geofence ${lastGeofenceID}`);
        const lastEnterState = await adapter.getStateAsync(`0_userdata.0.TimeTracking.${lastGeofenceID}.lastEnter`);
        if (lastEnterState && lastEnterState.val) {
            ...
            const hours = timeMs / 3600000;
            // Aktualisiere Zähler (Tag, Woche, Monat, Jahr)
            await addTimeToCounters(lastGeofenceID, timeMs);
            // Erstelle Log-Eintrag
            await createLogEntry(lastGeofenceID, hours);
        }
        // Setze lastEnter zurück
        await adapter.setStateAsync(`0_userdata.0.TimeTracking.${lastGeofenceID}.lastEnter`, { val: "", ack: true });
    }
    
    // F) Falls neuer Geofence vorhanden, Eintritt verarbeiten:
    if (newID) {
        adapter.log.info(`Betrete Geofence ${newID}`);
        await adapter.setStateAsync(`0_userdata.0.TimeTracking.${newID}.lastEnter`, { val: new Date().toISOString(), ack: true });
    }
    
    // G) Aktualisiere lastGeofenceID
...
function startAdapter(options) {
    ...
    adapter.on('ready', async () => {
        ...
        // Falls keine Konfiguration vorhanden ist, Standardwerte setzen
        ...
        // Abonniere den Geofence-Status (Beispiel: für Device 1)
    ...
    adapter.on('stateChange', async (id, state) => {
        if (id === "traccar.0.devices.1.geofences_string" && state) {
            // Hier rufen wir processGeofenceChange auf – in einer echten Umgebung würde oldState ggf. gesetzt sein
            processGeofenceChange({ state: state, oldState: {} }).catch(err => adapter.log.error(err));
        }
    });
